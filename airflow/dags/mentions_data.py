...
dag = DAG(
    'mentions_data', default_args=default_args, schedule_interval = '*/15 * * * *')
...
t2 = BashOperator(
    task_id='producer',
    bash_command="python /usr/local/kafka/airflow_producer_mentions.py {{ ti.xcom_pull(task_ids='print_date' )}}",
    retries=3,
    dag=dag)
